...
从零构建的 Linux 用户态，包括一个最小 C 标准库和一批自托管用户程序。运行于 x86_64 Linux，无 glibc 依赖。

项目包含自定义的 libc、pthread 实现（带异步线程资源回收机制）、内存池，以及一批自托管的用户态程序（shell、tmake、vim、网络工具等）。
...
- **注释**：添加注释时用英文。文件头部注释和索引用 `/* */` 块注释，函数前用 `/**` 块注释或 `/* */` 行内注释。禁止添加注释掉的死代码块
  ...
  **文件头注释** — 每个 .c 文件头部必须包含：
  ...
  - 第一行：程序名 + 短描述（一句话）
  - `机制`：程序围绕什么概念展开，最多两句话
  - `系统调用`：列举直接使用的系统调用（非 libc 包装）
  - `用法`：列出典型用法，无参数时就是最常用的
  - 权限、编译、特殊说明放在末尾

  **索引** — 紧跟文件头，列出关键函数或代码段的位置和意义：

  ```
  /*
   * 索引：
   *   main                   入口：解析参数，循环处理每个文件
   *     dump_file    行 42  打开 → 读取 → 写入完整循环
   *     resolve_path 行 78  处理特殊路径（"-" 转为 stdin）
   *   is_binary     行 105  检查文件是否包含空字节
   */
  ```
  ...
  - 一级函数顶格，子函数缩进
  - 出口函数（`main`）写"入口"
  - 行号标注核心逻辑的位置（不是声明的行号，是逻辑段起点的行号）
  - 对于简单程序（一个 main 函数完成所有工作）可以省略索引

  **核心逻辑注释** — 关键路径上标注"在做什么"，而不是"怎么做的"：

  ```c
  /* 逐文件处理循环 */
  for (int i = 1; i < argc; i++) {
      int fd = __open(argv[i], O_RDONLY);
      ...
  }
  ```
  ...
  ```c
  int n = __read(fd, buf, sizeof(buf));   /* 读取文件内容 */
  ```
