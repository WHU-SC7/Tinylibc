...
[feat] 实现异步回收线程栈和内存
1. 副作用是把以前的用户程序都干掉了。在main函数之后才让工作线程退出。之前不规范的做法是在main函数中直接exit，会留下工作线程一直不退出。
现在就改了shell.c pthread.c mempool.c。剩下的暂时不管
...
# 2026.5.28
增加默认编译选项和函数compile_file

# 2026.5.29
tmake可以编译并链接所有程序!

# 2026.5.30
可以安装程序,可以tab补全后执行~/tlibc/bin的程序
