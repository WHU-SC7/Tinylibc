...
//获取进程所属的用户目录，返回类似于"/home/username"的string
int tlibc_get_user_dir(char *user_dir_buf, int buf_size)
{
    uid_t uid = getuid();
    // printf("当前用户的UID是: %d\n", uid);
    ...
    strncpy(user_dir_buf, "/home/", buf_size - 1);
    strncat(user_dir_buf, usr_name, buf_size - 1);
    user_dir_buf[buf_size - 1] = 0;
    return 0;
}